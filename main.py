...
def crear_evento_calendario(titulo_proyecto: str, descripcion: str, correo_invitado: str, fecha: str, correo_asignador: str):
    ...
    rutas_posibles = ['/etc/secrets/google-credentials.json', '/opt/render/project/src/google-credentials.json', 'google-credentials.json']
    ...
    try:
        creds = service_account.Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=SCOPES).with_subject(correo_asignador)
...
# =================================================================
app = FastAPI(title="API Proeléctrica", version="4.0")
...
async def recibir_datos_formulario(payload: FormularioSolicitud, db: Session = Depends(get_db)):
    try:
        ...
        datos_dinamicos_json = {
            "tipo_registro": "Verificacion", "ubicacion": payload.ubicacion.model_dump(), "detalles_tecnicos": payload.detalles_tecnicos.model_dump(),
            "contacto": payload.contacto.model_dump(), "propietario": payload.propietario.model_dump(),
            "seguimiento_inspeccion": payload.seguimiento_inspeccion, "fecha_solicitud": fecha_limpia, "cancelacion_pago": "No", "moneda_cotizacion": "CRC"
        }
        nuevo_proyecto = ProyectoDB(empresa_encargada="UVIE Proeléctrica", empresa_solicitante=payload.empresa_solicitante, correo_solicitante=payload.correo_solicitante, datos_dinamicos=datos_dinamicos_json, estado="Nueva Solicitud")
...
async def actualizar_codigo_vba(payload: ActualizacionVBA, db: Session = Depends(get_db)):
    try:
        ...
        nuevo_log = {"id": int(time.time() * 1000), "autor": "Sistema (VBA Automático)", "texto": f"Se asignó el documento oficial: {payload.codigo_solicitud}. El estado cambió a 'Oferta Generada'.", "fecha": datetime.now().strftime("%d/%m/%Y %H:%M:%S")}
...
async def crear_proyecto_manual(db: Session = Depends(get_db)):
    try:
        datos_base = {
            "tipo_registro": "Proyecto", "ubicacion": {"provincia": "", "canton": "", "distrito": "", "exacta": ""},
            "detalles_tecnicos": {"actividad": "", "codigo_ciiu": "", "cantidad_permisos": "", "area_m2": ""}, "contacto": {"nombre": "", "telefono": ""},
            "propietario": {"nombre": "", "cedula": ""}, "fecha_solicitud": datetime.now().strftime("%Y-%m-%d"), "moneda_presupuesto": "CRC",
            "moneda_cotizacion": "CRC", "resultados_proyecto": "", "talento_requerido": [], "otro_talento": "", "colaboradores": []
        }
        nuevo_proyecto = ProyectoDB(titulo_proyecto="Nuevo Proyecto", empresa_encargada="Proeléctrica", empresa_solicitante="Cliente por definir", correo_solicitante="", datos_dinamicos=datos_base, estado="Cotización")
        db.add(nuevo_proyecto)
        db.commit()
        db.refresh(nuevo_proyecto)
        return {"status": "success", "id_proyecto": nuevo_proyecto.id}
...
async def eliminar_proyecto(id_proyecto: int, db: Session = Depends(get_db)):
    try:
        proyecto = db.query(ProyectoDB).filter(ProyectoDB.id == id_proyecto).first()
        if not proyecto: raise HTTPException(status_code=404, detail="Proyecto no encontrado")
        db.delete(proyecto)
...
async def crear_tarea(id_proyecto: int, payload: TareaCrear, db: Session = Depends(get_db)):
    try:
        ...
        nueva_tarea = TareaDB(id_proyecto=id_proyecto, descripcion=payload.descripcion, asignado_a=payload.asignado_a, asignado_por=payload.asignado_por, fecha_limite=payload.fecha_limite, enlace_calendario=enlace_cal)
        db.add(nueva_tarea)
        
        bitacora_actual = proyecto.bitacora if proyecto.bitacora else []
        nuevo_log = {"id": int(time.time() * 1000), "autor": payload.asignado_por, "texto": f"Asignó una nueva tarea a {payload.asignado_a}: '{payload.descripcion}' (Para el {payload.fecha_limite})", "fecha": datetime.now().strftime("%d/%m/%Y %H:%M:%S")}
        proyecto.bitacora = bitacora_actual + [nuevo_log]
        flag_modified(proyecto, "bitacora") 
        db.commit()
...
async def editar_tarea(id_tarea: int, payload: TareaActualizar, db: Session = Depends(get_db)):
    try:
        ...
        cambios = []
        if tarea.descripcion != payload.descripcion: cambios.append(f"Descripción (de '{tarea.descripcion}' a '{payload.descripcion}')"); tarea.descripcion = payload.descripcion
        if tarea.asignado_a != payload.asignado_a: cambios.append(f"Responsable (de '{tarea.asignado_a}' a '{payload.asignado_a}')"); tarea.asignado_a = payload.asignado_a
        if tarea.fecha_limite != payload.fecha_limite: cambios.append(f"Fecha (de '{tarea.fecha_limite}' a '{payload.fecha_limite}')"); tarea.fecha_limite = payload.fecha_limite
        ...
        if cambios:
            ...
            if proyecto:
                ...
                nuevo_log = {"id": int(time.time() * 1000), "autor": payload.modificado_por, "texto": f"Editó la tarea de inspección: Se modificó {texto_cambios}.", "fecha": datetime.now().strftime("%d/%m/%Y %H:%M:%S")}
                proyecto.bitacora = bitacora_actual + [nuevo_log]
                flag_modified(proyecto, "bitacora")
        db.commit()
        return {"status": "success"}
...
async def listar_mis_tareas(correo: str, db: Session = Depends(get_db)):
    resultados = db.query(TareaDB, ProyectoDB.titulo_proyecto, ProyectoDB.empresa_solicitante).outerjoin(ProyectoDB, TareaDB.id_proyecto == ProyectoDB.id).filter(TareaDB.asignado_a == correo, TareaDB.estado == 'Pendiente').order_by(TareaDB.fecha_limite.asc()).all()
    return [{"id": t.id, "proyecto": titulo or cliente or "Desconocido", "id_proyecto": t.id_proyecto, "descripcion": t.descripcion, "asignado_a": t.asignado_a, "asignado_por": t.asignado_por, "fecha_limite": t.fecha_limite, "enlace_calendario": t.enlace_calendario} for t, titulo, cliente in resultados]

@app.get("/v1/tareas/activas", status_code=200)
async def listar_todas_tareas_activas(db: Session = Depends(get_db)):
    resultados = db.query(TareaDB, ProyectoDB.titulo_proyecto, ProyectoDB.empresa_solicitante).outerjoin(ProyectoDB, TareaDB.id_proyecto == ProyectoDB.id).filter(TareaDB.estado == 'Pendiente').order_by(TareaDB.fecha_limite.asc()).all()
    return [{"id": t.id, "proyecto": titulo or cliente or "Desconocido", "id_proyecto": t.id_proyecto, "descripcion": t.descripcion, "asignado_a": t.asignado_a, "asignado_por": t.asignado_por, "fecha_limite": t.fecha_limite, "enlace_calendario": t.enlace_calendario} for t, titulo, cliente in resultados]
...
async def completar_tarea(id_tarea: int, db: Session = Depends(get_db)):
    try:
        ...
        if proyecto:
            bitacora_actual = proyecto.bitacora if proyecto.bitacora else []
            nuevo_log = {"id": int(time.time() * 1000), "autor": "Sistema", "texto": f"Se marcó como COMPLETADA la tarea: '{tarea.descripcion}'", "fecha": datetime.now().strftime("%d/%m/%Y %H:%M:%S")}
            proyecto.bitacora = bitacora_actual + [nuevo_log]
            flag_modified(proyecto, "bitacora") 
        db.commit()
        return {"status": "success"}
