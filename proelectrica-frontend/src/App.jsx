...
function App() {
  ...
  useEffect(() => { if (chatEndRef.current) chatEndRef.current.scrollIntoView({ behavior: "smooth", block: "end" }); }, [bitacora, modalAbierto]);
  ...
  const handleCrearTarea = async () => {
    ...
    try {
      ...
      cargarProyectos(); // Refresca la bitácora
  ...
  const abrirFicha = (proyecto) => {
    ...
    let colabArray = [];
    if (Array.isArray(proyecto.datos_dinamicos?.colaboradores)) colabArray = proyecto.datos_dinamicos.colaboradores;
    else if (proyecto.datos_dinamicos?.colaboradores) colabArray = [proyecto.datos_dinamicos.colaboradores];
  ...
  // Extraemos solo los nombres para los selects normales
            ...
            <TableContainer sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead sx={{ backgroundColor: '#f8fafc' }}>
                  {tabActual === 0 ? (
                    <TableRow><TableCell sx={{ ...tableHeadSx, width: '30%' }}>Cliente / Empresa</TableCell><TableCell sx={{ ...tableHeadSx, width: '20%' }}>ID Documento</TableCell><TableCell sx={{ ...tableHeadSx, width: '20%' }}>Status</TableCell><TableCell sx={{ ...tableHeadSx, width: '15%' }}>Empresa Encargada</TableCell><TableCell sx={{ ...tableHeadSx, width: '15%' }}>Área (m²)</TableCell></TableRow>
                  ) : (
          ...
          <>
            ...
            <DialogContent sx={{ padding: 0, display: 'flex', flexDirection: 'row', overflow: 'hidden' }}>
              ...
              <Box sx={{ width: '380px', flexShrink: 0, display: 'flex', flexDirection: 'column', borderLeft: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
                ...
                {/* SECCIÓN DE TAREAS */}
                <Box sx={{ flexShrink: 0, p: 2, borderBottom: '1px solid #e2e8f0', backgroundColor: '#f1f5f9' }}><Typography variant="subtitle1" sx={{ color: '#8b5cf6', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Tareas del Proyecto</Typography></Box>
                ...
                {tareasProyecto.length > 0 && (
                  <Box sx={{ p: 2, borderBottom: '1px solid #e2e8f0', maxHeight: '150px', overflowY: 'auto' }}>
                    <List disablePadding>
                      {tareasProyecto.map(t => (
                        <ListItem key={t.id} sx={{ px: 0, py: 0.5, opacity: t.estado === 'Completada' ? 0.6 : 1 }}>
                          <ListItemText
                            ...
                            secondary={<Typography variant="caption" color="textSecondary">{t.asignado_a.split('@')[0]} | {t.fecha_limite}</Typography>}
                          />
                      ...
                      return (
                        <ListItem key={comentario.id} alignItems="flex-start" sx={{ px: 0, mb: esSistema ? 0.5 : 2 }}>
                          ...
                          <ListItemText
                            ...
                            secondary={<Typography variant="body2" sx={{ mt: 0.5, color: esSistema ? '#6b7280' : '#111827', fontStyle: esSistema ? 'italic' : 'normal', backgroundColor: esSistema ? 'transparent' : '#fff', p: esSistema ? 0 : 1.5, border: esSistema ? 'none' : '1px solid #e2e8f0', borderRadius: '4px', fontSize: '0.8rem', wordBreak: 'break-word' }}>{comentario.texto}</Typography>}
                          />
                        </ListItem>
                      );
                    })}
